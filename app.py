...
cookie_mgr = stx.CookieManager()                 # prefix 인자 없음

VALID_USERS   = {"Korloy": "19660611"}           # 계정 1개
COOKIE_NAME   = "pvd_auth"                       # 접두사 포함된 쿠키명
COOKIE_TTL    = 30                               # 유지 일수
...
with tab1:
    ...
    if query:
        mask = raw_sorted.apply(
            lambda r: query.lower() in " ".join(r.astype(str)).lower(),
            axis=1
        )
...
with tab2:
    ...
    with c2:
        tmp = ref_df if alloy_pick == "전체" \
              else ref_df[ref_df["합금"] == alloy_pick]
    ...
    if key2:
        filt = filt[filt.apply(
            lambda r: key2.lower() in " ".join(r.astype(str)).lower(),
            axis=1
        )]
